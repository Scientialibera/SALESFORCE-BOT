...
def summarize_query_result(qr: Any, max_rows: int = 100) -> Dict[str, Any]:
...
async def run_test():
    print("Starting agentic chatbot test (run-until-done planning loop)")
    ...
    test_queries = [
        "How are you?",
        "Accounts that have SOWs similar to Microsoft's AI Chatbot engagements (i think the offer name is ai_chatbot) and then a way to contact the related account contacts",
        "All sales",
    ]
    ...
    try:
        ...
        for q in test_queries:
            ...
            while True:
                ...
                # Always pass the full planner toolset each round
                try:
                    ...
                    md_lines.append(f"**Planner call failed (round {round_num}):** {e}\n\n")
                    break

                md_lines.append(f"### Planner Request (Round {round_num})\n")
                ...
                md_lines.append(f"### Planner Raw Response (Round {round_num})\n")
                ...
                # Log and APPEND planner's assistant content to the conversation (preserve prior rounds)
                if planner_message.get("content"):
                    md_lines.append(f"### Planner Assistant Content (Round {round_num})\n")
                    md_lines.append(planner_message["content"] + "\n")
                    conversation_msgs.append({"role": "assistant", "content": planner_message["content"]})

                # If planner returned no tool calls, we are done for this query.
                ...
                md_lines.append(f"### Planner Tool Calls (Round {round_num})\n")
                ...
                for tc in planner_calls:
                    ...
                    # Parse planner arguments for the agent
                    try:
                    ...
                    # Optional: resolve account names
                    ...
                    # Call the agent (LLM) with its tools
                    ...
                    # Execute agent tool calls and collect compact summaries for planner reinjection
                ...
                # Inject THIS ROUND'S agent summaries back to the planner and CONTINUE THE SAME conversation (append, don't rebuild)
                ...
                if round_num >= max_rounds:
                    md_lines.append("**Reached max_rounds; stopping loop to avoid runaway planning.**\n\n")
                    break
